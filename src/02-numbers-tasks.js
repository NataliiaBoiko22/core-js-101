...
function getAngleBetweenVectors(x1, y1, x2, y2) {
  const partUp = x1 * x2 + y1 * y2;
  const partDown = Math.sqrt(x1 ** 2 + y1 ** 2) * Math.sqrt(x2 ** 2 + y2 ** 2);
  const body = partUp / partDown;
  return Math.acos(body);
}
