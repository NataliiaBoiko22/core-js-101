...
function doRectanglesOverlap(rect1, rect2) {
  ...
  const {
    left: x2, top: y2, width: w2, height: h2,
  } = rect2;
...
function isInsideCircle(circle, point) {
  ...
  return radius ** 2 > ((xPoint - xCenter) + (yPoint - yCenter)) ** 2;
}
...
function getCommonDirectoryPath(pathes) {
  ...
  const pathesDel = pathes
    .map((path) => path.split('/'));
  const commonDirectory = [];
  for (let i = 0; i < pathesDel[0].length; i += 1) {
    if (!pathesDel.every((path) => path[i] === pathesDel[0][i])) break;
    commonDirectory.push(`${pathesDel[0][i]}/`);
  }
...
function getMatrixProduct(m1, m2) {
  ...
  for (let k = 0; k < colsB; k += 1) {
    for (let i = 0; i < rowsA; i += 1) {
      let t = 0; for (let j = 0; j < rowsB; j += 1) t += m1[i][j] * m2[j][k]; C[i][k] = t;
    }
  }
  return C;
}
